...
ALTER TABLE users MODIFY COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время обновления строки";
...
-- 5.2 Подсчитайте средний возраст пользователей в таблице users.
...
-- 5.2 Подсчитайте количество дней рождения, которые приходятся на каждый из дней недели. Следует учесть, что необходимы дни недели текущего года, а не года рождения.

 select 
    weekday(date_format(birthday_at, '2021-%m-%d')) as 'day',
    count(*) as 'birthdays'
from users
